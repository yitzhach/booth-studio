...
async function boot() {
  ...
  const btn = (action, label, ic, cls = "") =>
    `<button data-action="${action}" class="${cls}" title="${e(label)}">${ic ? icon(ic) : ""}<span>${label}</span></button>`;
  let p,
    selected = null,
    photoSelected = null,
    tab = "art",
  ...
  function libraryHTML(mobile = false) {
    return `<div class="panel-heading"><h2>Artwork library <small>${p.art.length}</small></h2>${btn("upload-art", "Add artwork", "plus", "icon-only")}</div>${!mobile ? '<label class="search"><span>⌕</span><input id="search" placeholder="Find artwork…" aria-label="Find artwork" value="' + e(search) + '"/></label>' : ""}<div class="library-caption">ORIGINALS & PANELS <span>inches</span></div><div class="art-list">${
      p.art
        ...
        .map(
          (a) =>
            `<button class="art-card ${a.id === selected ? "selected" : ""}" data-art="${a.id}"><div class="thumb">${artThumb(a)}</div><div><strong>${e(a.title)}</strong><span>${a.w} × ${a.h} × ${a.thickness} in</span><small>${a.asset ? a.wall + " wall" : "Dimension placeholder"}</small></div></button>`,
        )
        .join("") || '<p class="muted">Upload JPG or PNG artwork to begin.</p>'
    }</div><div class="library-bottom">${btn("upload-art", "Upload artwork", "image-plus", "wide")}${p.art.some((a) => !a.asset) ? btn("clear-samples", "Remove sample panels", null, "wide text-button") : ""}<p>Original images stay intact.<br>Saved only on this browser/device.</p></div>`;
  }
  ...
  const actions = {
    ...
    "duplicate-art": () =>
      mutate(() => {
        ...
        if (!a) return;
        const copy = { ...a, id: uid(), title: a.title + " copy", x: a.x + 6 };
        p.art.push(copy);
        selected = copy.id;
      }),
    "delete-art": () =>
