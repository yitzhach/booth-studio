...
export function makeTent(W,D,H,style='classic') {
 ...
 const steel=material('#c0c4c5',{metalness:.65,roughness:.32}),fabric=material('#f7f6f0',{side:T.DoubleSide}),seam=material('#deddd5');
...
export function environment(scene,g,b){const kind=b.ground||'studio',setting=b.horizon||'studio';scene.background=new T.Color(setting==='studio'?'#b5b4b0':'#c8dce7');scene.fog=setting==='studio'?null:new T.Fog('#c8dce7',24,75);
 ...
 const floor=mesh(g,new T.PlaneGeometry(180,180),material(kind==='studio'?'#a9a6a0':'#ffffff',{map:textures.get(kind)||null}));floor.rotation.x=-Math.PI/2;floor.position.y=-.045;
 if(setting==='park')for(let i=0;i<24;i++){const angle=i/24*Math.PI*2,dist=19+(i%4)*3,x=Math.cos(angle)*dist,z=Math.sin(angle)*dist;const trunk=mesh(g,new T.CylinderGeometry(.17,.24,3.5,7),material('#625746'));trunk.position.set(x,1.7,z);for(let j=0;j<3;j++){const crown=mesh(g,new T.SphereGeometry(1.6+j*.12,12,8),material(['#687951','#75855b','#536b48'][j]));crown.position.set(x+Math.sin(i+j)*.75,3.8+j*.65,z+Math.cos(i+j)*.65);crown.scale.y=1.1;}}
 if(setting==='urban')for(let i=0;i<9;i++){const h=4+(i%3)*1.7,m=mesh(g,new T.BoxGeometry(5,h,4),material(['#b2afa5','#a09f99','#c3baaa'][i%3]));m.position.set((i-4)*6,h/2,-24);}
 if(b.neighbors)for(const x of [-1,1])for(const z of [0,-5.5]){const t=makeTent(3.048,3.048,2.44,'classic');t.position.set(x*(b.width*.0254/2+2.2),0,z);g.add(t);const wall=mesh(g,new T.BoxGeometry(3,.0+2.2,.04),material('#d6d1c5'));wall.position.set(t.position.x,1.1,z-1.5);}
}
